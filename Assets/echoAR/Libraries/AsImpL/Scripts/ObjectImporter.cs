...
namespace AsImpL
{
    ...
    public class ObjectImporter : MonoBehaviour
    {
        ...
        public void ImportModelAsync(Entry entry, string objName, string filePath, string texName, Transform parentObj, ImportOptions options)
        {
            this.entry = entry;
        ...
        protected virtual void OnImported(GameObject obj, string absolutePath)
        {
            ...
            if (!string.IsNullOrEmpty(this.texName))
            {
                ...
                texture.LoadImage(File.ReadAllBytes(Application.persistentDataPath + "/" + this.texName));
